...
class AmmImpl {
    ...
    async claimLockFee2(owner, maxAmount, payer, receiver) {
        const [lockEscrowPK] = (0, utils_1.deriveLockEscrowPda)(this.address, owner, this.program.programId);
        let tokenAOwner = receiver;
        let tokenBOwner = receiver;
        if (this.tokenAMint.address.equals(spl_token_1.NATIVE_MINT)) {
            tokenAOwner = owner;
        }
        if (this.tokenBMint.address.equals(spl_token_1.NATIVE_MINT)) {
            tokenBOwner = owner;
        }
        const preInstructions = [];
        const [[userAta, createUserAtaIx], [escrowAta, createEscrowAtaIx], [tokenAAta, createTokenAAtaIx], [tokenBAta, createTokenBAtaIx],] = await Promise.all([
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.lpMint, owner, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.lpMint, lockEscrowPK, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.tokenAMint, tokenAOwner, this.program.provider.connection, payer),
            (0, utils_1.getOrCreateATAInstruction)(this.poolState.tokenBMint, tokenBOwner, this.program.provider.connection, payer),
        ]);
        createUserAtaIx && preInstructions.push(createUserAtaIx);
        createEscrowAtaIx && preInstructions.push(createEscrowAtaIx);
        createTokenAAtaIx && preInstructions.push(createTokenAAtaIx);
        createTokenBAtaIx && preInstructions.push(createTokenBAtaIx);
        const postInstructions = [];
        if (this.poolState.tokenAMint.equals(spl_token_1.NATIVE_MINT) || this.poolState.tokenBMint.equals(spl_token_1.NATIVE_MINT)) {
            // unwarp sol to receiver
            const closeWrappedSOLIx = await (0, utils_1.unwrapSOLInstruction)(owner, receiver);
            closeWrappedSOLIx && postInstructions.push(closeWrappedSOLIx);
        }
        const accounts = {
            pool: this.address,
